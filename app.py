...
# ── MathJax preload for instant LaTeX rendering ─────────────────────────────
st.markdown("""
<script>
window.MathJax = {
  tex: {
...
<style>
/* ── Instant loading splash ── */
#mwa-splash{position:fixed;inset:0;z-index:99999;background:#0b0f1a;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.2rem;transition:opacity 0.5s ease;}
#mwa-splash.hidden{opacity:0;pointer-events:none;}
.mwa-logo{font-family:'Sora',sans-serif;font-size:1.6rem;font-weight:800;background:linear-gradient(90deg,#4f8ef7,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
.mwa-dots{display:flex;gap:0.5rem;}
.mwa-dot{width:8px;height:8px;border-radius:50%;background:#4f8ef7;animation:mwaBounce 1.2s infinite ease-in-out;}
.mwa-dot:nth-child(2){background:#a78bfa;animation-delay:0.2s;}
.mwa-dot:nth-child(3){background:#34d399;animation-delay:0.4s;}
@keyframes mwaBounce{0%,80%,100%{transform:scale(0.8);opacity:0.5}40%{transform:scale(1.2);opacity:1}}
</style>
<div id="mwa-splash">
  <div class="mwa-logo">📐 MathWithAmrit</div>
  <div class="mwa-dots">
    <div class="mwa-dot"></div><div class="mwa-dot"></div><div class="mwa-dot"></div>
  </div>
  <div style="color:#4b5563;font-size:0.8rem;font-family:'Sora',sans-serif;">Loading calculus engine…</div>
</div>
<script>
(function(){
  function hideSplash(){
    var s=document.getElementById('mwa-splash');
    if(s){s.classList.add('hidden');setTimeout(function(){s.remove();},600);}
  }
  if(document.readyState==='complete'){setTimeout(hideSplash,400);}
  else{window.addEventListener('load',function(){setTimeout(hideSplash,400);});}
  setTimeout(hideSplash,3500); // hard cap
})();
</script>
...
# ║  CSS                                                                     ║
# ╚══════════════════════════════════════════════════════════════════════════╝
# ── Inject font preload in <head> for maximum speed (avoids @import blocking) ──
st.markdown("""
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link rel="preload" as="style" href="https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;700&display=swap">
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;700&display=swap" media="print" onload="this.media='all'">
<noscript><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;700&display=swap"></noscript>
""", unsafe_allow_html=True)
...
def analyse_expression(expr_str: str, var_str: str) -> dict:
    ...
    ALL_TRIG = TRIG | HYP | INV
    is_quotient = is_composite = False
    for n in nodes:
        if not is_quotient and isinstance(n, Pow):
            exp_ = n.args[1]
            if exp_ == -1 or (isinstance(exp_, sp.Integer) and exp_ < -1): is_quotient = True
        if not is_composite:
...
def _cached_critical_pts(expr_str, var_str):
    """Return real zeros of the derivative (critical points where f'=0)."""
    var = symbols(var_str); expr = sympify(expr_str)
    try:
        d_expr = diff(expr, var)
        pts = sp.solve(d_expr, var, rational=False)
...
def final_card(body_fn, extra_latex: str = ""):
    st.markdown('<div class="final-card">', unsafe_allow_html=True)
    st.markdown("### ✅ Final Answer")
    body_fn()
    if extra_latex:
        # Show copy hint with the raw LaTeX string
        st.markdown(
            f'<div style="margin-top:0.6rem;padding:0.5rem 0.8rem;background:rgba(0,0,0,0.2);'
            f'border-radius:8px;font-family:\'JetBrains Mono\',monospace;font-size:0.72rem;'
            f'color:#4b5563;word-break:break-all;">LaTeX: <span style="color:#64748b;">{extra_latex}</span></div>',
            unsafe_allow_html=True
        )
    st.markdown('</div>', unsafe_allow_html=True)
...
def solve_derivative(expr, var, order, show_formula, show_graph):
    ...
    def _ans():
        suf = {1:"'",2:"''",3:"'''"}.get(order,f"^{{({order})}}")
        st.latex(rf"f{suf}(x)={latex(simplified)}")
    final_card(_ans, extra_latex=latex(simplified))
...
def solve_integral(expr, var, is_def, lower, upper, show_formula, show_graph):
    ...
    if is_def:
        ...
        final_card(_ans, extra_latex=rf"\int_{{{lo_lt}}}^{{{hi_lt}}} {flt}\,d{v} = {latex(res_simp)}")
    else:
        res_simp = anti_simp; num_val = None
        def _ans(): st.latex(rf"\int {flt}\,d{v} = {latex(anti_simp)} + C")
        final_card(_ans, extra_latex=rf"\int {flt}\,d{v} = {latex(anti_simp)} + C")
...
def solve_double_integral(expr, v1, v2, lim1, lim2, show_formula, show_graph):
    ...
    _ans_latex = (rf"\iint_R {latex(expr)}\,d{n1}\,d{n2} = {latex(final_s)}" if lim1 and lim2
                  else rf"\iint {latex(expr)}\,d{n1}\,d{n2} = {latex(final_s)} + C")
    final_card(_ans, extra_latex=_ans_latex)
...
def solve_triple_integral(expr, v1, v2, v3, lim1, lim2, lim3, show_formula, show_graph):
    ...
    _flt = latex(expr)
    _ans_latex_t = (rf"\iiint_V {_flt}\,dV = {latex(r3s)}" if lim1 and lim2 and lim3
                    else rf"\iiint {_flt}\,d{n1}\,d{n2}\,d{n3} = {latex(r3s)} + C")
    final_card(_ans, extra_latex=_ans_latex_t)
...
# Warm up SymPy on first load — exercises all four solver code-paths
@st.cache_resource(show_spinner=False)
def _warmup():
    import sympy as _sp
    x, y, z = _sp.symbols('x y z')
    # Derivative paths: power, trig, chain, product
    _sp.diff(_sp.sin(x)*_sp.exp(x), x)
    _sp.diff(_sp.log(_sp.sin(x)), x)
    _sp.diff(x**3 + x**2, x, 2)
    # Single integral paths: direct, u-sub, definite, FToC
    _sp.integrate(_sp.sin(x), x)
    _sp.integrate(x*_sp.exp(x), x)
    _sp.integrate(_sp.sin(x), (x, 0, _sp.pi))
    _sp.simplify(_sp.integrate(x**2, x))
    # Double integral
    _sp.integrate(_sp.integrate(x*y, x), y)
    # Triple integral
    _sp.integrate(_sp.integrate(_sp.integrate(x*y*z, x), y), z)
    # Pre-build preorder_traversal cache
    e = _sp.sin(x**2)*_sp.exp(x)
    list(_sp.preorder_traversal(e))
    return True
_warmup()
...
if st.session_state.operation is None:
    ...
    <div class="section-heading">
        ...
        <p>Type these expressions in the input box — rendered as beautiful math below</p>
    </div>""", unsafe_allow_html=True)

    st.markdown(r"""
    <style>
    .guide-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1rem;margin-bottom:2rem;}
    ...
    .guide-card-basic{--gc:#4f8ef7;}.guide-card-trig{--gc:#a78bfa;}.guide-card-exp{--gc:#34d399;}.guide-card-adv{--gc:#fb923c;}.guide-card-multi{--gc:#f43f5e;}
    .guide-card h4{font-size:0.75rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--gc);margin:0 0 0.8rem;}
    ...
    .guide-table td{padding:0.48rem 0.3rem;font-size:0.82rem;color:var(--text-body);vertical-align:middle;line-height:1.5;}
    .guide-table td.math-col{width:42%;text-align:left;padding-left:0;}
    .guide-table td.code-col{width:58%;}
    .guide-code{font-family:'JetBrains Mono',monospace;background:color-mix(in srgb,var(--gc) 12%,transparent);color:var(--gc);border:1px solid color-mix(in srgb,var(--gc) 25%,transparent);padding:0.18rem 0.55rem;border-radius:6px;font-size:0.8rem;font-weight:700;}
    .guide-arrow{color:var(--text-muted);font-size:0.7rem;padding:0 0.25rem;}
    /* MathJax inside guide tables */
    .guide-table .MJX-TEX,.guide-table mjx-container{font-size:0.98em!important;color:var(--text-head)!important;}
    ...
    <div class="guide-grid">
      ...
      <div class="guide-card guide-card-basic">
        <h4>⚡ Powers &amp; Arithmetic</h4>
        <table class="guide-table">
          <tr>
            <td class="math-col">\(x^2\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^3\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**3</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^n\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**n</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x \cdot y\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x*y</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\dfrac{x}{y}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x/y</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sqrt{x}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sqrt(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\dfrac{1}{x^2+1}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">1/(x**2+1)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^{2} + 3x - 1\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2+3*x-1</span></td>
          </tr>
        </table>
      </div>
      ...
      <div class="guide-card guide-card-trig">
        ...
        <table class="guide-table">
          <tr>
            <td class="math-col">\(\sin x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\cos x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">cos(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\tan x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">tan(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\arcsin x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">asin(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\arccos x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">acos(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\arctan x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">atan(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sin^2 x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x)**2</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sin(x^2)\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x**2)</span></td>
          </tr>
        </table>
      </div>
      ...
      <div class="guide-card guide-card-exp">
        ...
        <table class="guide-table">
          <tr>
            <td class="math-col">\(e^x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">exp(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\ln x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">log(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\log_{10} x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">log(x,10)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sinh x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sinh(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\cosh x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">cosh(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\tanh x\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">tanh(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(e^{x^2}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">exp(x**2)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\ln(\sin x)\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">log(sin(x))</span></td>
          </tr>
        </table>
      </div>
      ...
      <div class="guide-card guide-card-adv">
        <h4>🌟 Special Values &amp; Constants</h4>
        <table class="guide-table">
          <tr>
            <td class="math-col">\(\infty\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">oo</span></td>
          </tr>
          <tr>
            <td class="math-col">\(-\infty\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">-oo</span></td>
          ...
          <tr>
            <td class="math-col">\(\pi\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">pi</span></td>
          </tr>
          <tr>
            <td class="math-col">\(e\) (Euler)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">E</span></td>
          </tr>
          <tr>
            <td class="math-col">\(|x|\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">Abs(x)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x \sin(x^2)\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x*sin(x**2)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\dfrac{\sin x}{x^2+1}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x)/(x**2+1)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^2 e^{-x}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2*exp(-x)</span></td>
          </tr>
        </table>
      </div>

      <!-- MULTI-VAR -->
      <div class="guide-card guide-card-multi">
        <h4>🌐 Multi-Variable (Double &amp; Triple)</h4>
        <table class="guide-table">
          <tr>
            <td class="math-col">\(x \cdot y\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x*y</span></td>
          ...
          <tr>
            <td class="math-col">\(x^2 y\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2*y</span></td>
          </tr>
          <tr>
            <td class="math-col">\(e^{x+y}\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">exp(x+y)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sin(x+y)\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x+y)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^2+y^2+z^2\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2+y**2+z**2</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x \cdot y \cdot z\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x*y*z</span></td>
          </tr>
          <tr>
            <td class="math-col">\(\sin(x)\cos(y)\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">sin(x)*cos(y)</span></td>
          </tr>
          <tr>
            <td class="math-col">\(x^2 y + yz^2\)</td>
            <td class="code-col"><span class="guide-arrow">→</span> <span class="guide-code">x**2*y+y*z**2</span></td>
          </tr>
        </table>
      </div>
    ...
    <script>
    // Trigger MathJax to typeset the guide tables after they're injected into the DOM
    (function tryMJ(){
      if(window.MathJax && window.MathJax.typesetPromise){
        window.MathJax.typesetPromise();
      } else {
        setTimeout(tryMJ, 200);
      }
    })();
    </script>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        # Determine the default function text clearly
        if sel != "Custom" and sel in ex:
            default = ex[sel]
        elif "Triple" in operation:
            default = "x*y*z"
        elif "Double" in operation:
            default = "x*y"
        else:
            default = "x**2"
        ...
        if func_in.strip():
            try:
                _prev_expr  = sympify(func_in.strip())
                _prev_latex = latex(_prev_expr)
                st.markdown(
                    f'<div style="background:var(--surface2);border:1px solid var(--border);'
                    f'border-left:3px solid var(--accent2);border-radius:10px;padding:0.6rem 1rem;'
                    f'margin-top:-0.3rem;margin-bottom:0.5rem;">'
                    f'<div style="font-size:0.68rem;font-weight:700;letter-spacing:0.1em;'
                    f'text-transform:uppercase;color:var(--accent2);margin-bottom:0.25rem;">Preview</div>'
                    f'<span style="color:var(--text-head);">\\(\\displaystyle {_prev_latex}\\)</span>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                # Trigger MathJax typeset in a separate markdown call so it is never stripped
                st.markdown(
                    '<script>(function tryMJ(){if(window.MathJax&&window.MathJax.typesetPromise)'
                    '{window.MathJax.typesetPromise();}else{setTimeout(tryMJ,150);}})();</script>',
                    unsafe_allow_html=True
                )
            except Exception:
                pass  # Invalid syntax — don't show preview
    ...
    with col2:
        ...
        if render_solution:
            ...
            try:
                ...
                if solve_btn:
                    st.markdown(
                        f'<div style="text-align:right;font-size:0.72rem;color:#4b5563;'
                        f'font-family:\'JetBrains Mono\',monospace;margin-top:0.5rem;">'
                        f'⚡ Computed in {_elapsed:.3f}s</div>',
                        unsafe_allow_html=True
                    )
                ...
                border-radius:10px;padding:0.8rem 1.2rem;font-size:0.83rem;color:#94a3b8;margin-top:0.5rem;">
                <strong style="color:#fb923c;">💡 Syntax tips:</strong><br>
                • Use <code>log(x)</code> for natural log (ln x)<br>
                • Use <code>**</code> for powers: <code>x**2</code> = x²<br>
                • Use <code>oo</code> for infinity (∞)<br>
                • Use <code>*</code> for multiplication: <code>x*sin(x)</code><br>
                • Parentheses are required: <code>sin(x)</code> not <code>sinx</code>
                </div>
                """, unsafe_allow_html=True)
...
<div class="footer-wrap">
    <strong>MathWithAmrit</strong> &nbsp;·&nbsp; Advanced Calculus Solver &nbsp;·&nbsp;
    Built for mathematics learners
    <br>
    <span style="font-size:0.72rem;opacity:0.6;">Powered by SymPy · Plotly · Streamlit</span>
</div>
<button onclick="window.scrollTo({top:0,behavior:'smooth'})"
  style="position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;
         ...
         border:none;border-radius:50%;width:42px;height:42px;font-size:1.1rem;
         cursor:pointer;box-shadow:0 4px 18px rgba(79,142,247,0.45);
         display:flex;align-items:center;justify-content:center;
         transition:transform 0.2s,box-shadow 0.2s;"
  onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 8px 28px rgba(79,142,247,0.6)'"
  onmouseout="this.style.transform='';this.style.boxShadow='0 4px 18px rgba(79,142,247,0.45)'"
